...
from keras.layers import GlobalAveragePooling2D, Dense, Cropping2D
...
def build_linknet_bottleneck_crop(
        backbone,
        decoder_block,
        skip_connection_layers,
        decoder_filters=(256, 128, 64, 32, 16),
    ...
    x2 = Dense(classification_classes, activation=classification_activation, name="classification_output")(x2)
    ...
    # model head (define number of output classes)
    x = layers.Conv2D(
        filters=segmentation_classes,
        kernel_size=(3, 3),
        padding='same',
        use_bias=True,
        kernel_initializer='glorot_uniform'
    )(x)
    x = layers.Activation(segmentation_activation, name="segmentation_output")(x)

    # create keras model instance
    model = models.Model(input_, [x,x2])
...
def Linknet_bottleneck_crop(
    ...
    model = build_linknet_bottleneck_crop(
        backbone=backbone,
        decoder_block=decoder_block,
        skip_connection_layers=encoder_features,
        decoder_filters=decoder_filters,
        ...
        segmentation_activation=segmentation_activation,
        n_upsample_blocks=len(decoder_filters),
        use_batchnorm=decoder_use_batchnorm,
    )
